...
export default async function handler(req: AuthRequest, res: VercelResponse) {
	...
	if (req.method === 'GET') {
		...
		try {
			const conn = await getConnection()
			await conn.query( 'UPDATE Product SET name = ?, price = ?, description = ?, material = ?, keyword = ?, category = ?, imageurl = ? WHERE productid = ?', [name, price, description ?? '', material ?? '', keyword ?? '', category ?? '', imageurl ?? '', id] )
